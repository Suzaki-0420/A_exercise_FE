...
export class EmployeeAccountRepository implements IEmployeeAccountRepository {
  ...
  public async getForm(): Promise<EmployeeAccount[]> {
    ...
    return responseData.employees.map((employee) => ({
      ...
      } as NonNullable<EmployeeAccount["employee"]>,
    }));
  }
  ...
  public async existsByAccountName(accountName: string): Promise<boolean> {
    ...
    const url = `/proxy-api/account/validate?${params.toString()}`;
  ...
  public async create(
    ...
    const requestBody = {
      employeeUuid: employeeAccount.employee?.employeeUuid,
    ...
    return {
      accountUuid: responseData.accountUuid ?? "",
