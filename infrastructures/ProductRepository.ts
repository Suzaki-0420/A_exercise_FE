...
export class ProductRepository implements IProductRepository {
  ...
  public async findById(productUuid: string): Promise<Product | null> {
    const url = `/proxy-api/product/edit/${encodeURIComponent(productUuid)}`;
  ...
  public async selectByProductCategoryId(
     ...
     */
    if (categoryUuid) {
      params.append("productCategoryUuid", categoryUuid);
    }

    params.append("showDeletedOnly", String(showDeletedOnly));
    ...
    if (!response.ok) {
      ...
      console.error("search statusText:", response.statusText);
  ...
  public async existsByName(name: string): Promise<void> {
    ...
    const url = `/proxy-api/product/validate?${params.toString()}`;
   ...
   * 商品を登録する
   */
  public async register(product: Product, imageFile: File): Promise<Product> {
     ...
     */
    requestFormData.append("name", product.name);

    requestFormData.append("price", String(product.price));
    ...
    requestFormData.append("categoryName", product.productCategory?.name ?? "");
    ...
    if (!response.ok) {
       ...
       * Entriesを配列にしてログへ出す。
       */
      console.log("request form data:", Array.from(requestFormData.entries()));
  ...
  public async updateById(
    ...
    const url = `/proxy-api/product/edit/${encodeURIComponent(product.productUuid)}`;

    const requestBody = new FormData();
    requestBody.append("Name", product.name);
    requestBody.append("Price", String(product.price));
    requestBody.append(
      "StockQuantity",
      String(product.productStock?.quantity ?? 0),
    );
   ...
   * 商品を削除する
   */
  public async deleteById(productUuid: string): Promise<boolean> {
    // TODO: 実際のControllerのURLに合わせて修正する
    const url = `/proxy-api/product/delete/${productUuid}`;
    ...
    if (!response.ok) {
      ...
      throw new Error(`商品の削除に失敗しました (Status: ${response.status})`);
    }
