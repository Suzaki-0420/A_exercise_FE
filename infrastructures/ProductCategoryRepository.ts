...
export class ProductCategoryRepository implements IProductCategoryRepository {
   ...
   */
  public async findById(categoryUuid: string): Promise<ProductCategory | null> {
    /*
     * 商品カテゴリ1件取得APIがないため、
     * 一覧取得後にUUIDで検索する。
     */
    ...
    return (
      categories.find((category) => category.categoryUuid === categoryUuid) ??
      null
    );
  }
  ...
  public async existsByName(name: string): Promise<void> {
    ...
    // TODO: 実際のControllerのURLに合わせて修正する
    const url = `/proxy-api/category/validate?${params.toString()}`;
  ...
  public async create(
    ...
    // TODO: 実際のControllerのURLに合わせて修正する
    const url = "/proxy-api/category/register";
    ...
    if (!response.ok) {
      ...
      console.log("========== REGISTER PRODUCT CATEGORY ==========");
