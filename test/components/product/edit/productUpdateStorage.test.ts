...
describe("productUpdateStorage", () => {
  ...
  it("選択した商品を保存して同じUUIDで取得できる", () => {
    ...
    expect(loadProductForUpdate(product.productUuid)).toEqual(product);
  });
  ...
  it("URLの商品UUIDと保存商品のUUIDが異なる場合は取得しない", () => {
    ...
    expect(
      loadProductForUpdate("10000000-0000-0000-0000-000000000002"),
    ).toBeNull();
  });
  ...
  it("画像URL・カテゴリ・在庫がnullでも商品を復元できる", () => {
    ...
    expect(loadProductForUpdate(productWithoutRelations.productUuid)).toEqual(
