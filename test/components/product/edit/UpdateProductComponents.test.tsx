...
describe("商品修正コンポーネント", () => {
    ...
    it("入力値・画像・項目エラーと送信エラーを表示する", () => {
        ...
        expect(
            screen.getByText("選択中：change.png")
        ).toBeTruthy();
        expect(
            screen.getByAltText(
                "水性ボールペン黒の変更後の商品画像"
        ...
        expect(
            (screen.getByLabelText("単価") as HTMLInputElement)
                .value
        ).toBe("");
        expect(
            (screen.getByLabelText("在庫数") as HTMLInputElement)
                .value
        ).toBe("");
    });
    ...
    it("確認情報がない場合は商品検索へ戻す", () => {
        ...
        expect(
            hookState.handleInvalidFlow
        ).toHaveBeenCalledWith("/admin/product");
        expect(screen.getByRole("status").textContent).toContain(
            "商品検索画面へ移動しています..."
        );
    });
    ...
    it("確認画面に送信・項目エラー、変更画像、初期値を表示する", () => {
        ...
        expect(
            screen.getByText("変更後：change.png")
        ).toBeTruthy();
        expect(
            screen.getByAltText(
                "水性ボールペン黒の変更後の商品画像"
    ...
    it("完了情報を表示してメニュー・商品検索への遷移をHookへ渡す", () => {
        ...
        expect(
            screen.getByText(`対象商品：${product.name}`)
        ).toBeTruthy();
        fireEvent.click(
            screen.getByRole("button", {
                name: "メニューへ",
        ...
        ).toHaveBeenNthCalledWith(1, "/admin");
        expect(
            hookState.handleLeaveComplete
...
describe("UpdateProductContext", () => {
    ...
    it("入力内容・画像・完了結果を保存し、フローを初期化できる", () => {
        ...
        expect(result.current.completedResult).toEqual(
            completedResult
        );
    ...
    it("Provider外でContextを利用した場合はエラーにする", () => {
        expect(() =>
            renderHook(() => useUpdateProductContext())
        ).toThrow(
