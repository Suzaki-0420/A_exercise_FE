...
const changeField = (result: HookResult, name: string, value: string) => {
  act(() => {
...
const submitForConfirmation = async (result: HookResult) => {
  await act(async () => {
...
const renderInitializedHook = async (selectedProduct: Product = product) => {
  saveProductForUpdate(selectedProduct);
  const hook = renderHook(() => useUpdateProduct(selectedProduct.productUuid));

  await waitFor(() => {
    expect(hook.result.current.isLoading).toBe(false);
    expect(hook.result.current.formData).toEqual(selectedProduct);
  });
...
describe("useUpdateProductの商品修正フロー", () => {
  beforeEach(() => {
    ...
    mockGetCategories.mockResolvedValue([category, otherCategory]);
  ...
  it("商品未選択の各操作は安全に終了し、指定された遷移だけを行う", async () => {
    ...
    act(() => {
      ...
      result.current.handleLeaveComplete("/admin/product");
    });

    expect(result.current.formData).toBeNull();
    expect(mockRouter.replace).toHaveBeenCalledWith("/admin/product");
    expect(contextState.clearFlow).toHaveBeenCalledTimes(2);
    expect(mockRouter.push).toHaveBeenCalledWith("/admin/product");
  });
  ...
  it("Contextの商品と画像を初期値として利用する", async () => {
    ...
    const { result } = renderHook(() => useUpdateProduct(product.productUuid));
    ...
    expect(result.current.categories).toEqual([category, otherCategory]);
  });
  ...
  it("商品カテゴリ取得の一般エラーを画面へ表示する", async () => {
    ...
    const { result } = renderHook(() =>
      useUpdateProduct(product.productUuid, { initialProduct: product }),
    );
  ...
  it("初期化完了前に画面を離れた場合は状態を更新しない", async () => {
    let resolveCategories!: (categories: ProductCategory[]) => void;
    ...
    saveProductForUpdate(product);
    const { unmount } = renderHook(() => useUpdateProduct(product.productUuid));
  ...
  it("各入力値を画面状態へ反映し、フォーカス離脱時に検証する", async () => {
    ...
    changeField(result, "categoryUuid", otherCategory.categoryUuid);
  ...
  it("在庫情報がない商品でも在庫数を入力できる", async () => {
    ...
    const { result } = await renderInitializedHook(productWithoutStock);
  ...
  it("選択画像のプレビューURLを生成し、破棄時に解放する", async () => {
    ...
    const { result, unmount } = await renderInitializedHook();
    const imageFile = new File(["image"], "product.png", {
      type: "image/png",
    });
  ...
  it("確認済みの商品名は重複確認APIを再度呼ばない", async () => {
    ...
    expect(mockValidateProductName).toHaveBeenCalledTimes(1);
  });
  ...
  it("異なる商品名の重複確認が並行した場合は新しい確認状態を維持する", async () => {
    ...
    act(() => {
      firstValidation = result.current.handleNameBlur();
    });
    changeField(result, "name", "蛍光ペン");
    let secondValidation!: Promise<void>;
    act(() => {
      secondValidation = result.current.handleNameBlur();
    });
    ...
    expect(mockValidateProductName).toHaveBeenCalledTimes(2);
    expect(result.current.fieldErrors.name).toBeUndefined();
  });
  ...
  it("入力エラーがある場合は確認画面へ進まない", async () => {
    ...
    expect(result.current.fieldErrors.name).toBe("商品名を入力してください。");
  ...
  it("モーダルでは選択商品を初期表示して確認画面を開く", async () => {
    ...
    expect(contextState.saveDraft).toHaveBeenCalledWith(product, null);
  ...
  it("モーダルの商品更新成功時は完了処理を呼び出す", async () => {
    ...
    expect(onUpdateSuccess).toHaveBeenCalledWith(updateResult);
    expect(onUpdatePendingChange.mock.calls).toEqual([[true], [false]]);
    expect(mockRouter.push).not.toHaveBeenCalledWith(
  ...
  it("更新後処理の失敗を商品更新APIの失敗と区別する", async () => {
    ...
    contextState.draft = product;
    const { result } = renderHook(() =>
      useUpdateProduct(undefined, { onUpdateSuccess }),
    );
  ...
  it("商品更新成功時は完了結果を保存して完了画面へ進む", async () => {
    ...
    expect(mockUpdateProduct).toHaveBeenCalledWith(product, imageFile);
    expect(contextState.setCompletedResult).toHaveBeenCalledWith(updateResult);
    expect(loadProductForUpdate(product.productUuid)).toBeNull();
  ...
  it("商品更新の業務エラーを項目・送信エラーへ反映する", async () => {
    contextState.draft = product;
    mockUpdateProduct.mockRejectedValue(
      new ProductUpdateError("入力内容を確認してください。", 400, {
        name: "商品名エラー",
      }),
    );
    const { result } = renderHook(() => useUpdateProduct());
    ...
    expect(result.current.fieldErrors.name).toBe("商品名エラー");
    expect(result.current.submitError).toBe("入力内容を確認してください。");
  });
