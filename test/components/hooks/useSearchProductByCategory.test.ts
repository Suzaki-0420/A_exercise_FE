...
} from "@/components/hooks/useSearchProductByCategory";
...
describe(
    ...
    () => {
         ...
         */
        it(
            "カテゴリUUIDが空文字の場合空文字のままServiceへ渡す",
            async () => {
                mockExecute
                    .mockResolvedValueOnce(
                        products
         ...
         */
        it(
            "ServiceがErrorを投げた場合messageを設定する",
            async () => {
                const serviceError =
                    new Error(
                        "カテゴリ検索エラー"
