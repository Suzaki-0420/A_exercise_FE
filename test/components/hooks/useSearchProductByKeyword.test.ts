...
vi.mock("@/di/types", () => ({
  TYPES: {
    ISearchProductByKeywordService: Symbol("ISearchProductByKeywordService"),
  },
}));
 ...
 * キーワード商品検索Hookのテスト
 */
describe("useSearchProductByKeyword", () => {
  const products = [
    {
   ...
   */
  it("初期状態が正しい", () => {
    const { result } = renderHook(() => useSearchProductByKeyword());
  ...
  it("ServiceをDIコンテナから取得する", () => {
    renderHook(() => useSearchProductByKeyword());
   ...
   * 検索成功
   */
  it("キーワード検索に成功した場合商品一覧を設定する", async () => {
    mockExecute.mockResolvedValueOnce(products);

    const { result } = renderHook(() => useSearchProductByKeyword());

    await act(async () => {
      await result.current.search("ノート", false);
    });

    expect(mockExecute).toHaveBeenCalledWith("ノート", false);
   ...
   * 空文字キーワード
   */
  it("キーワードが空文字の場合も空文字のままServiceへ渡す", async () => {
    mockExecute.mockResolvedValueOnce(products);
   ...
   * 削除済み商品のみ検索
   */
  it("削除済み商品のみ検索する場合trueをServiceへ渡す", async () => {
    mockExecute.mockResolvedValueOnce(products);

    const { result } = renderHook(() => useSearchProductByKeyword());

    await act(async () => {
      await result.current.search("ノート", true);
    });

    expect(mockExecute).toHaveBeenCalledWith("ノート", true);
  });

  /**
   * Error発生
   */
  it("ServiceがErrorを投げた場合messageを設定し商品一覧を空にする", async () => {
    const { result } = renderHook(() => useSearchProductByKeyword());
     ...
     * 次の検索を失敗させる。
     */
    mockExecute.mockRejectedValueOnce(new Error("キーワード検索エラー"));

    await act(async () => {
      await result.current.search("ペン", false);
    });

    expect(result.current.error).toBe("キーワード検索エラー");
  ...
  it("ServiceがError以外を投げた場合既定メッセージを設定する", async () => {
    ...
    expect(result.current.error).toBe("商品の検索に失敗しました。");
  ...
  it("検索処理中はisLoadingがtrueになる", async () => {
    ...
    await act(async () => {
      searchPromise = result.current.search("ノート", false);
  ...
  it("再検索を開始すると以前のエラーをnullにする", async () => {
    ...
    await act(async () => {
      await result.current.search("失敗", false);
    });
    ...
    await act(async () => {
      searchPromise = result.current.search("再検索", false);
  ...
  it("検索結果が0件の場合空配列を設定する", async () => {
    ...
    await act(async () => {
      await result.current.search("存在しない商品", false);
    });
