...
const createImageFile = (overrides: Partial<File> = {}): File =>
  ({
    name: "product.png",
    size: 1024,
    type: "image/png",
...
describe("isProductUuid", () => {
  it("サンプルデータのUUID形式を有効と判定する", () => {
    expect(isProductUuid("10000000-0000-0000-0000-000000000001")).toBe(true);
  });

  it("空のUUIDと不正な文字列を無効と判定する", () => {
    expect(isProductUuid("00000000-0000-0000-0000-000000000000")).toBe(false);
...
describe("validateUpdateProduct", () => {
  ...
  it("価格と数量が上限を超えた場合は範囲エラーを返す", () => {
    ...
    expect(result).toMatchObject({
      ...
      stockQuantity: "数量は1000個以下で入力してください。",
    });
  });
  ...
  it.each([
    ...
    expect(validateUpdateProduct(createProduct({ name })).name).toBe(
      "商品名は2～20文字で入力してください。",
    );
  });
  ...
  it("使用できない記号を含む商品名では文字種エラーを返す", () => {
    expect(
      validateUpdateProduct(createProduct({ name: "ボールペン@黒" })).name,
    ).toBe("商品名は全角・半角英数字で入力してください。");
  });
  ...
  it("負数と小数は数値形式エラーを返す", () => {
    ...
    expect(negativeResult).toMatchObject({
      ...
      stockQuantity: "正しい数量形式で入力してください。",
    });
    expect(decimalResult).toEqual(negativeResult);
  });

  it("対応形式で2MB以下の画像を有効と判定する", () => {
    const result = validateUpdateProduct(createProduct(), createImageFile());
  ...
  it("2MBを超える画像では容量エラーを返す", () => {
    ...
    expect(result.image).toBe("画像のファイルサイズは2MB以下にしてください。");
  });
  ...
  it("空の画像ファイルではエラーを返す", () => {
    ...
    expect(result.image).toBe("画像ファイルが空です。");
  });
