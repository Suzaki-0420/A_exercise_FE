...
describe("RegisterCategoryService", () => {
    ...
    beforeEach(() => {
        ...
        service = new RegisterCategoryService(
            repository,
        );
    });
    ...
    it(
        "カテゴリー名の重複確認をするとRepositoryのexistsByNameが呼ばれる",
        async () => {
            // データを用意する
            const categoryName = "文房具";

            vi.mocked(repository.existsByName)
                .mockResolvedValue(undefined);

            // カテゴリー名の重複確認を実行する
            await service.validateCategoryName(
                categoryName,
            );
    ...
    it(
        "カテゴリー名の重複確認でRepositoryが例外を発生させるとその例外が伝播する",
        async () => {
            // データを用意する
            const categoryName = "文房具";
            const error =
                new Error("カテゴリー名が重複しています。");
    ...
    it(
        "カテゴリーを登録するとRepositoryのcreateが呼ばれ登録結果が返される",
        async () => {
            // データを用意する
    ...
    it(
        "カテゴリー登録でRepositoryが例外を発生させるとその例外が伝播する",
        async () => {
            // データを用意する
            const productCategory =
                {} as ProductCategory;
