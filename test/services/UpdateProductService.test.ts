...
const createProductRepository = (updateResult = true): IProductRepository => ({
...
const createCategoryRepository = (): IProductCategoryRepository => ({
  findAll: async () => [category],
  findById: async (categoryUuid) =>
    categoryUuid === category.categoryUuid ? category : null,
  existsByName: async () => undefined,
...
describe("UpdateProductService", () => {
  it("商品名の重複確認をRepositoryへ委譲する", async () => {
    ...
    const existsByName = vi.spyOn(repository, "existsByName");
    ...
    expect(existsByName).toHaveBeenCalledWith("油性ペン");
  });
  ...
  it("商品を修正して完了画面用の結果を返す", async () => {
    ...
    await expect(service.updateProduct(product)).resolves.toEqual({
  ...
  it("選択した画像をRepositoryへ渡す", async () => {
    const repository = createProductRepository();
    const updateById = vi.spyOn(repository, "updateById");
    const service = new UpdateProductService(
      repository,
      createCategoryRepository(),
    );
    ...
    expect(updateById).toHaveBeenCalledWith(product, imageFile);
  });
