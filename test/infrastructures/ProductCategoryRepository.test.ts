...
describe("ProductCategoryRepository", () => {
  ...
  describe("findAll", () => {
    it("商品カテゴリ一覧を取得して返す", async () => {
      ...
      fetchMock.mockResolvedValue(createResponse(categories));
    ...
    it("messageがある場合はmessageをthrowする", async () => {
      ...
      await expect(repository.findAll()).rejects.toThrow("取得できません");
    });
    ...
    it("errorsがある場合はerrors内容をthrowする", async () => {
      ...
      await expect(repository.findAll()).rejects.toThrow("カテゴリ名エラー");
    });
  ...
  describe("findById", () => {
    it("存在するカテゴリの場合はカテゴリを返す", async () => {
      ...
      expect(result?.categoryUuid).toBe("target-id");
    });

    it("存在しないカテゴリの場合はnullを返す", async () => {
      fetchMock.mockResolvedValue(createResponse([createCategory()]));
  ...
  describe("existsByName", () => {
    it("カテゴリ名が存在しない場合は正常終了する", async () => {
      ...
      await expect(repository.existsByName("食品")).resolves.toBeUndefined();
    });
    ...
    it("messageエラーの場合はthrowする", async () => {
      ...
      await expect(repository.existsByName("食品")).rejects.toThrow(
        "すでに登録されています",
      );
    });
  ...
  describe("create", () => {
    ...
    it("エラーレスポンスのjson解析に失敗した場合は既定エラーをthrowする", async () => {
      ...
      await expect(repository.create(createCategory())).rejects.toThrow(
