...
describe("ProductRepository", () => {
  ...
  describe("findAll", () => {
    ...
    it("正しいURL・HTTPメソッド・ヘッダー・credentialsで通信する", async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith("/proxy-api/product/category", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  describe("findById", () => {
    it("商品が存在する場合は商品を返す", async () => {
      ...
      fetchMock.mockResolvedValue(createResponse(product, { status: 200 }));

      const result = await repository.findById("product-uuid");
    ...
    it("fetchが失敗した場合は例外をそのまま投げる", async () => {
      ...
      await expect(repository.findById("product-uuid")).rejects.toThrow(error);
    });
    ...
    it("レスポンスのJSON解析に失敗した場合は例外を投げる", async () => {
      ...
      await expect(repository.findById("product-uuid")).rejects.toThrow(
        "JSON Error",
      );
    });
  ...
  describe("searchKeyword", () => {
    it("該当する商品が存在する場合は商品配列を返す", async () => {
      ...
      const result = await repository.searchKeyword("りんご", false);
    ...
    it("該当する商品が存在しない場合は空配列を返す", async () => {
      ...
      const result = await repository.searchKeyword("存在しない商品", false);
    ...
    it("HTTPエラーの場合はステータスを含む例外を投げる", async () => {
      ...
      await expect(repository.searchKeyword("りんご", false)).rejects.toThrow(
    ...
    it("キーワードとshowDeletedOnly=trueをURLに含める", async () => {
      ...
      await repository.searchKeyword("削除済み", true);

      const expectedUrl = `/proxy-api/product/keyword?keyword=${encodeURIComponent(
        "削除済み",
  ...
  describe("selectByProductCategoryId", () => {
    it("カテゴリに属する商品が存在する場合は商品配列を返す", async () => {
      ...
      const result = await repository.selectByProductCategoryId(
        "category-uuid",
        false,
      );
    ...
    it("カテゴリUUIDが空文字の場合はproductCategoryUuidを送信しない", async () => {
      ...
      await repository.selectByProductCategoryId("", false);
    ...
    it("HTTPエラーの場合はステータスを含む例外を投げる", async () => {
      ...
      await expect(
        repository.selectByProductCategoryId("category-uuid", false),
      ).rejects.toThrow("商品カテゴリによる検索に失敗しました (Status: 500)");
    });
    ...
    it("HTTPエラー時にstatusTextが未定義でも例外を投げる", async () => {
      ...
      await expect(
        repository.selectByProductCategoryId("category-uuid", false),
      ).rejects.toThrow("商品カテゴリによる検索に失敗しました (Status: 400)");
    });
    ...
    it("カテゴリUUIDとshowDeletedOnly=falseをURLに含める", async () => {
      ...
      await repository.selectByProductCategoryId("category+uuid", false);

      const expectedUrl = `/proxy-api/product/category?productCategoryUuid=${encodeURIComponent(
        "category+uuid",
    ...
    it("カテゴリUUIDとshowDeletedOnly=trueをURLに含める", async () => {
      ...
      await repository.selectByProductCategoryId("category-uuid", true);
    ...
    it("カテゴリUUIDに特殊文字がある場合URLエンコードする", async () => {
      ...
      await repository.selectByProductCategoryId("category uuid/test", false);
  ...
  describe("existsByName", () => {
    it("正常レスポンスの場合は例外を投げずに終了する", async () => {
      ...
      await expect(repository.existsByName("りんご")).resolves.toBeUndefined();
    });
    ...
    it("messageを含むエラーの場合はmessageの内容で例外を投げる", async () => {
      ...
      await expect(repository.existsByName("りんご")).rejects.toThrow(
        "商品名は既に登録されています",
      );
    });
  ...
  describe("register", () => {
    ...
    it("正常に商品を登録できる", async () => {
      ...
      fetchMock.mockResolvedValue(createResponse(responseProduct));

      const result = await repository.register(product, createFile());
    ...
    it("正しいURLとPOSTメソッドでFormDataを送信する", async () => {
      ...
      await repository.register(product, createFile());

      expect(fetchMock).toHaveBeenCalled();

      const [url, options] = fetchMock.mock.calls[0];

      expect(url).toBe("/proxy-api/product/register");
    ...
    it("商品情報をFormDataへ格納する", async () => {
      ...
      expect(formData.get("name")).toBe("りんご");

      expect(formData.get("price")).toBe("100");

      expect(formData.get("stock")).toBe("10");

      expect(formData.get("categoryUuid")).toBe("category-uuid");

      expect(formData.get("categoryName")).toBe("食品");
    });
    ...
    it("在庫情報がない場合はstockに0を格納する", async () => {
      ...
      expect(formData.get("stock")).toBe("0");
    });
    ...
    it("カテゴリ情報がない場合は空文字を格納する", async () => {
      ...
      expect(formData.get("categoryUuid")).toBe("");

      expect(formData.get("categoryName")).toBe("");
    });
    ...
    it("画像がある場合はimageをFormDataへ格納する", async () => {
      ...
      await repository.register(product, file);
    ...
    it("画像がない場合はimageをFormDataへ格納しない", async () => {
      ...
      await repository.register(product, null as unknown as File);
    ...
    it("errorsの値が配列の場合はvalidation例外を投げる", async () => {
      ...
      await expect(
        repository.register(createProduct(), createFile()),
      ).rejects.toThrow(
        JSON.stringify({
          type: "validation",
          errors: {
    ...
    it("errorsのフィールド名の先頭を小文字へ変換する", async () => {
      ...
      await expect(
        repository.register(createProduct(), createFile()),
      ).rejects.toThrow('"stock":"在庫数が不正です"');
    });
    ...
    it("messageもerrorsもない場合は既定の例外を投げる", async () => {
      ...
      await expect(
        repository.register(createProduct(), createFile()),
      ).rejects.toThrow("商品の登録に失敗しました (Status: 500)");
    });
  ...
  describe("updateById", () => {
    it("正常更新の場合はtrueを返す", async () => {
      ...
      const result = await repository.updateById(createProduct());
    ...
    it("FormDataに商品情報を含める", async () => {
      ...
      expect(body.get("Name")).toBe(product.name);
      expect(body.get("Price")).toBe(String(product.price));
      expect(body.get("StockQuantity")).toBe(
        String(product.productStock?.quantity),
      );
      expect(body.get("CategoryUuid")).toBe(
    ...
    it("選択した画像をImageとしてFormDataへ追加する", async () => {
      const imageFile = new File(["image-data"], "product.png", {
        type: "image/png",
      });

      fetchMock.mockResolvedValue(createResponse({}));

      await repository.updateById(createProduct(), imageFile);
    ...
    it("validationエラーの場合はvalidation内容をthrowする", async () => {
      ...
      await expect(repository.updateById(createProduct())).rejects.toThrow(
        "商品名は必須です",
      );
    });
    ...
    it("在庫情報がない場合はStockQuantityを0で送信する", async () => {
      ...
      expect(body.get("StockQuantity")).toBe("0");
    });
  ...
  describe("deleteById", () => {
    it("正常削除の場合はtrueを返す", async () => {
      ...
      const result = await repository.deleteById("product-uuid");
    ...
    it("削除対象が存在しない場合はfalseを返す", async () => {
      ...
      const result = await repository.deleteById("not-found");
    ...
    it("DELETEメソッドで正しいURLへ通信する", async () => {
      ...
      await repository.deleteById("product-uuid");
    ...
    it("HTTPエラーの場合は例外をthrowする", async () => {
      ...
      await expect(repository.deleteById("product-uuid")).rejects.toThrow(
        "削除に失敗しました",
      );
    });
