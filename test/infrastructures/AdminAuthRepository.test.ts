...
import { AdminLoginError } from "@/models/AdminAuth";
...
describe("AdminAuthRepository", () => {
  ...
  describe("login", () => {
    it("ログイン成功", async () => {
      ...
      await expect(repository.login(credentials)).resolves.toEqual(loginUser);
    });
    ...
    it("errors配列messageを返す", async () => {
      ...
      await expect(repository.login(credentials)).rejects.toThrow("認証失敗");
    });
    ...
    it("errors配列内を検索してmessageを取得する", async () => {
      ...
      await expect(repository.logout()).rejects.toThrow("2件目のエラー");
    });
    ...
    it("messageエラー", async () => {
      ...
      await expect(repository.login(credentials)).rejects.toThrow("エラー");
    });

    it("400エラー", async () => {
      fetchMock.mockResolvedValue(createErrorResponse({}, 400));
    ...
    it("401エラー", async () => {
      fetchMock.mockResolvedValue(createErrorResponse({}, 401));
    ...
    it("403エラー", async () => {
      fetchMock.mockResolvedValue(createErrorResponse({}, 403));
    ...
    it("field errorsを取得する", async () => {
      ...
      try {
        ...
        expect(error).toBeInstanceOf(AdminLoginError);

        expect((error as AdminLoginError).fieldErrors).toEqual({
          accountName: "必須です",
          password: "必須です",
        });
      }
  ...
  describe("logout", () => {
    ...
    it("errors message", async () => {
      ...
      await expect(repository.logout()).rejects.toThrow("logout失敗");
    });
    ...
    it("messageエラー", async () => {
      ...
      await expect(repository.logout()).rejects.toThrow("失敗");
    });

    it("401の場合", async () => {
      fetchMock.mockResolvedValue(createErrorResponse({}, 401));

      await expect(repository.logout()).rejects.toThrow("ログインの有効期限");
    });
