...
const ORDER_SEARCH_PATH =
    "/proxy-api/order/search";

const ORDER_SEARCH_RESULT_PATH =
    "/proxy-api/order/search/result";
 ...
 */
const openOrderSearchPage = async (
    page: Page
...
const toDateInputValue = (
    ...
    const matchedDate = orderDate.match(
        /\d{4}[-/]\d{2}[-/]\d{2}/
    );

    return matchedDate
        ? matchedDate[0].replaceAll(
            "/",
            "-"
        )
        : null;
...
test.describe(
    ...
    () => {
        test(
            "初期表示で全購入履歴と検索項目を表示する",
            async ({ page }) => {
                const orders =
                    await openOrderSearchPage(
                        page
                    );
        ...
        test(
            "該当注文がない場合はデータがないことを表すメッセージを表示する",
            async ({ page }) => {
                await openOrderSearchPage(
                    page
                );
            ...
            async ({ page }) => {
                const orders = Array.from(
                    ...
                    (_, index): OrderSearchItem => ({
                        ...
                        orderStatus: "注文済",
                        statusUpdateUrl:
                            `/admin/order/status/update/e2e-order-${index + 1}`,
                    })
                );
