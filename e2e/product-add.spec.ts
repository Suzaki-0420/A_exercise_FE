 ...
 * 確認モーダルのボタン名
 */
const MODAL_REGISTER_BUTTON_NAME =
    "確認";

const MODAL_CANCEL_BUTTON_NAME =
    "キャンセル";

/**
 * 商品登録画面を開く
 */
const openProductAddPage = async (
    page: Page
): Promise<void> => {
    await page.goto(
        "/admin/product/add"
...
test.describe(
    ...
    () => {
         ...
         */
        test(
            "未入力の項目からフォーカスを外すと必須エラーが表示され確認モーダルは開かない",
            async ({ page }) => {
                const {
                    productNameInput,
                    priceInput,
         ...
         */
        test(
            "正常な値を入力して完了すると確認モーダルが表示される",
            async ({ page }) => {
                await fillValidProductForm(
                    page
                );
         ...
         */
        test(
            "確認モーダルでキャンセルするとモーダルが閉じ入力値が保持される",
            async ({ page }) => {
                await fillValidProductForm(
                    page
                );
         ...
         */
        test(
            "確認モーダルで登録すると商品登録が完了し商品検索画面へ遷移する",
            async ({ page }) => {
                /*
