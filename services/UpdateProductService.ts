...
export class UpdateProductService implements IUpdateProductService {
   ...
   * 商品名の重複を確認する
   */
  public async validateProductName(name: string): Promise<void> {
  ...
  public async updateProduct(
    ...
    const updated = await this.productRepository.updateById(product, imageFile);

    if (!updated) {
      throw new ProductUpdateError("指定された商品は存在しません。", 404);
    }
    ...
    return {
      ...
      name: product.name,
      price: product.price,
      stockQuantity: product.productStock?.quantity ?? 0,
