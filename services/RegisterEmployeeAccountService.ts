...
export class RegisterEmployeeAccountService implements IRegisterEmployeeAccountService {
  ...
  public constructor(
    ...
    private readonly employeeAccountRepository: IEmployeeAccountRepository,
  ) {}

  /**
   * アカウント未登録社員一覧を取得する
   */
  public async getForm(): Promise<EmployeeAccount[]> {
    return await this.employeeAccountRepository.getForm();
  }

  /**
   * アカウント名の重複確認をする
   */
  public async validateAccountName(accountName: string): Promise<void> {
    const normalizedAccountName = accountName.trim();

    const exists = await this.employeeAccountRepository.existsByAccountName(
  ...
  public async registerEmployeeAccount(
    ...
    const normalizedEmployeeAccount: EmployeeAccount = {
      ...employeeAccount,
      name: employeeAccount.name.trim(),
    };

    return await this.employeeAccountRepository.create(
      normalizedEmployeeAccount,
    );
  }
}
