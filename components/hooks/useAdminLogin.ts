...
const validateCredentials = (
  ...
  if (!credentials.accountName) {
    errors.accountName = "アカウント名を入力してください。";
  } else if (
    credentials.accountName.length < 5 ||
    credentials.accountName.length > 20
  ) {
    errors.accountName = "アカウント名は5～20文字で入力してください。";
  } else if (!halfWidthAlphaNumericPattern.test(credentials.accountName)) {
    errors.accountName = "アカウント名は半角英数字で入力してください。";
  }
  ...
  if (!credentials.password) {
    ...
    errors.password = "パスワードは5～20文字で入力してください。";
  } else if (!halfWidthAlphaNumericPattern.test(credentials.password)) {
...
export const useAdminLogin = () => {
  ...
  const service = useMemo(
    () => container.get<ILoginAdminService>(TYPES.ILoginAdminService),
    [],
  );

  const [credentials, setCredentials] =
    useState<AdminLoginCredentials>(initialCredentials);
  ...
  const handleChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    const field = event.target.name as keyof AdminLoginCredentials;
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        const loggedInAdmin = await service.login(credentials);
