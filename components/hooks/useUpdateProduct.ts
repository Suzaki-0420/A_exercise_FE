...
const PRODUCT_CONFIRM_PATH = "/admin/product/edit/confirm";
const PRODUCT_COMPLETE_PATH = "/admin/product/edit/complete";
...
export type UseUpdateProductOptions = {
  ...
  onUpdateSuccess?: (result: ProductUpdateResult) => void | Promise<void>;
  onUpdatePendingChange?: (isPending: boolean) => void;
  onBackToInput?: () => void;
  onCancel?: () => void;
};

const createImagePreviewUrl = (imageFile: File | null): string | null => {
  if (
    !imageFile ||
...
const revokeImagePreviewUrl = (previewUrl: string | null): void => {
  if (
    previewUrl &&
    typeof URL !== "undefined" &&
...
export const useUpdateProduct = (
  ...
  const service = useMemo(
    () => container.get<IUpdateProductService>(TYPES.IUpdateProductService),
    [],
  );
  const {
    draft,
  ...
  const [formData, setFormData] = useState<Product | null>(
    draft?.productUuid === productUuid
      ? draft
      : initialProduct?.productUuid === productUuid
        ? (initialProduct ?? null)
        : null,
  );
  const [categories, setCategories] = useState<ProductCategory[]>([]);
  const initialImageFile =
    draft && (!productUuid || draft.productUuid === productUuid)
      ? draftImageFile
      : null;
  const [imageFile, setImageFile] = useState<File | null>(initialImageFile);
  const [imagePreviewUrl, setImagePreviewUrl] = useState<string | null>(null);
  const [fieldErrors, setFieldErrors] = useState<ProductUpdateFieldErrors>({});
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(Boolean(productUuid));
  const originalProductNameRef = useRef<string | null>(null);
  const currentProductNameRef = useRef("");
  const validatedProductNameRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    const previewTimer = window.setTimeout(() => {
      nextPreviewUrl = createImagePreviewUrl(imageFile);
  ...
  useEffect(() => {
    ...
    const initializeProduct = async () => {
      ...
      const storedProduct = loadProductForUpdate(productUuid);
      ...
      originalProductNameRef.current = originalProduct.name.trim();
      currentProductNameRef.current = selectedProduct.name;
      validatedProductNameRef.current = originalProduct.name.trim();

      try {
        const productCategories = await service.getCategories();
  ...
  }, [draft, initialProduct, productUuid, router, service]);
        ...
        switch (name) {
          ...
          case "stockQuantity":
            return {
              ...previous,
              productStock: {
                stockUuid: previous.productStock?.stockUuid ?? "",
                quantity: value === "" ? Number.NaN : Number(value),
              },
            };
          case "categoryUuid": {
            const category =
              categories.find((item) => item.categoryUuid === value) ?? null;
            return {
              ...previous,
  ...
  const validateField = useCallback(
    (fieldName: keyof ProductUpdateFieldErrors) => {
      ...
      const validationErrors = validateUpdateProduct(formData, imageFile);
  ...
  const validateName = useCallback(async (): Promise<boolean> => {
    ...
    const validationError = validateUpdateProduct(formData, imageFile).name;
    ...
    if (
      normalizedName === originalProductNameRef.current ||
      normalizedName === validatedProductNameRef.current
    ) {
      setFieldErrors((previous) => ({
        ...previous,
        name: undefined,
    ...
    const pendingValidation = pendingNameValidationRef.current;

    if (pendingValidation?.name === normalizedName) {
      return await pendingValidation.promise;
    }

    const validationPromise = (async () => {
      try {
        await service.validateProductName(normalizedName);

        if (currentProductNameRef.current.trim() !== normalizedName) {
          return false;
        }

        validatedProductNameRef.current = normalizedName;
        setFieldErrors((previous) => ({
          ...previous,
          name: undefined,
        }));
        ...
        if (pendingNameValidationRef.current?.name === normalizedName) {
          pendingNameValidationRef.current = null;
        }
      }
    })();
  ...
  const handleProceedToConfirm = useCallback(
    ...
    [formData, imageFile, router, saveDraft, validateName, onProceedToConfirm],
  );
  ...
  const handleUpdate = useCallback(async () => {
    ...
    try {
      result = await service.updateProduct(draft, imageFile);
    } catch (error: unknown) {
      setIsLoading(false);
  ...
  const handleBackToInput = useCallback(() => {
    ...
    router.push(`/admin/product/edit/${encodeURIComponent(draft.productUuid)}`);
