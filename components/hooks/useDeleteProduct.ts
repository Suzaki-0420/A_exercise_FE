...
export const useDeleteProduct = () => {
   ...
   * 削除完了通知
   */
  const [isDeleteToastVisible, setIsDeleteToastVisible] =
    useState<boolean>(false);

  const service = useMemo<IDeleteProductService>(
    () => container.get<IDeleteProductService>(TYPES.IDeleteProductService),
    [],
  );
  ...
  const confirmDelete = useCallback(async (): Promise<boolean> => {
    ...
    try {
      const result = await service.execute(deleteTarget.productUuid);

      if (!result) {
        throw new Error("商品を削除できませんでした。");
      }
      ...
      const message =
        e instanceof Error ? e.message : "商品の削除に失敗しました。";
