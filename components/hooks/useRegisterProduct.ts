...
export const useRegisterProduct = () => {
  ...
  const [formData, setFormData] = useState<Product>(createInitialFormData);
   ...
   * 画面表示用に統合する
   */
  const displayErrors = useMemo<ProductFormErrors>(() => {
    const mergedErrors = {
      ...errors,
    };

    if (categoriesError) {
      mergedErrors.system = categoriesError;
    } else {
  ...
  const validateName = useCallback(async (): Promise<boolean> => {
    ...
    if (!namePattern.test(name)) {
      setErrors((prev) => ({
        ...prev,
        name: "商品名は日本語または全角・半角英数字で入力してください。",
      }));
   ...
   * 単価を検証する
   */
  const validatePrice = useCallback((): boolean => {
  ...
  const validateStock = useCallback((): boolean => {
    const stock = formData.productStock?.quantity;
  ...
  const validateImage = useCallback((): boolean => {
    ...
    if (!allowedTypes.includes(imageFile.type)) {
      setErrors((prev) => ({
        ...prev,
        image: "商品画像はJPEGまたはPNG形式を選択してください。",
      }));
  ...
  const validateImageDimensions = useCallback(
    async (file: File): Promise<boolean> => {
      return new Promise((resolve) => {
        ...
        image.onload = () => {
          const isValid = image.width <= 1000 && image.height <= 1000;
          ...
          if (!isValid) {
            setErrors((prev) => ({
              ...prev,
              image: "商品画像は縦横1000px以下を選択してください。",
            }));
  ...
  const handleChange = useCallback((event: ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData((prev) => ({
      ...
      [name]:
        name === "price" ? (value === "" ? Number.NaN : Number(value)) : value,
    }));

    setErrors((prev) => {
      const newErrors = { ...prev };

      delete newErrors[name as keyof ProductFormErrors];
  ...
  const handleImageChange = useCallback(
    ...
    [validateImageDimensions, imagePreviewUrl],
  );
  ...
  const handleRegisterProduct =
    useCallback(async (): Promise<Product | null> => {
      ...
      try {
         ...
         */
        return await service.registerProduct(formData, imageFile);
      } catch (error: unknown) {
        const message =
        ...
        try {
          ...
          if (parsed.type === "validation") {
            const convertedErrors: ProductFormErrors = {};
            ...
            Object.entries(parsed.errors).forEach(([key, value]) => {
               ...
               * imageとして格納される。
               */
              convertedErrors[normalizedKey as keyof ProductFormErrors] =
                String(value);
            });
  ...
  const confirmRegisterProduct = useCallback(async () => {
    const result = await handleRegisterProduct();
