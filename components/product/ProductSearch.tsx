 ...
 * 現在画面に表示する検索結果の種類
 */
type DisplayMode = "category" | "keyword";
...
export const ProductSearch = () => {
  ...
  const handleConfirmDelete = async (): Promise<void> => {
    ...
    if (displayMode === "keyword") {
      ...
      await searchByKeyword(trimmedKeyword, showDeletedOnly);

      return;
    }

    const categoryUuidForSearch = categoryUuid === "all" ? "" : categoryUuid;

    await searchByCategory(categoryUuidForSearch, showDeletedOnly);
  };
   ...
   */
  useEffect(() => {
    void searchByCategory("", false);
  }, [searchByCategory]);

  /**
   * 検索方法タブ切り替え
   */
  const handleTabChange = (mode: DisplayMode) => {
    setDisplayMode(mode);
  };
  ...
  const handleSearchClick = () => {
    ...
    void searchByKeyword(trimmedKeyword, showDeletedOnly);
  };
   ...
   * カテゴリ未指定の検索を行う。
   */
  const handleCategoryChange = (selectedCategoryUuid: string) => {
    setCategoryUuid(selectedCategoryUuid);

    setDisplayMode("category");

    const valueForApi =
      selectedCategoryUuid === "all" ? "" : selectedCategoryUuid;

    void searchByCategory(valueForApi, showDeletedOnly);
  };
   ...
   * 通常商品と削除済み商品を切り替える。
   */
  const handleDeletedOnlyChange = (checked: boolean) => {
    ...
    if (displayMode === "keyword") {
      ...
      void searchByKeyword(trimmedKeyword, checked);
    ...
    void searchByCategory(categoryUuidForSearch, checked);
  };
   ...
   */
  const isLoading =
    displayMode === "keyword" ? isKeywordLoading : isCategoryLoading;

  /**
   * 現在表示中の検索処理のエラー
   */
  const error = displayMode === "keyword" ? keywordError : categoryError;

  const isDeletedCheckboxDisabled =
    isLoading || (displayMode === "category" && categoryUuid === "all");
  ...
  const handleUpdate = (product: Product) => {
    ...
    router.push(`/admin/product/edit/${product.productUuid}`);
  };

  return (
    <div className="mx-auto w-full max-w-6xl rounded-lg border border-border bg-white p-8 shadow-sm">
      <div className="mb-6 flex items-center justify-center gap-4">
        <h2 className="text-2xl font-bold text-foreground">商品検索</h2>
      ...
      >
        <button
          type="button"
          role="tab"
          aria-selected={displayMode === "category"}
          onClick={() => {
            handleTabChange("category");
          }}
          className={`rounded-t-md border px-4 py-3 font-semibold transition-colors ${
        ...
        <button
          type="button"
          role="tab"
          aria-selected={displayMode === "keyword"}
          onClick={() => {
              ...
              <div className="flex flex-col gap-1">
                <div
                  className={`flex items-center space-x-2 ${
                    isDeletedCheckboxDisabled ? "opacity-50" : ""
                  }`}
                >
                  <Checkbox
                    id="showDeletedOnlyCategory"
                    checked={showDeletedOnly}
                    disabled={isDeletedCheckboxDisabled}
                    onCheckedChange={(checked) =>
                      handleDeletedOnlyChange(checked === true)
                    }
                  />
            ...
            <div className="mt-5 rounded-md border border-gray-200 bg-gray-50 p-4">
              <dl className="space-y-3">
                <div className="grid grid-cols-[100px_1fr] gap-3">
                  <dt className="font-semibold text-gray-700">商品名</dt>
                ...
                <div className="grid grid-cols-[100px_1fr] gap-3">
                  <dt className="font-semibold text-gray-700">単価</dt>

                  <dd className="text-gray-900">
                    {deleteTarget.price.toLocaleString()}円
                  </dd>
                </div>

                <div className="grid grid-cols-[100px_1fr] gap-3">
                  <dt className="font-semibold text-gray-700">カテゴリ</dt>

                  <dd className="break-words text-gray-900">
                    {deleteTarget.productCategory?.name ?? "未設定"}
                  </dd>
                </div>
              </dl>
            </div>
