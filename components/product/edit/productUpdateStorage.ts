 ...
 */
export const saveProductForUpdate = (product: Product): void => {
  const storage = getSessionStorage();

  if (!storage) {
    throw new Error("商品修正用の一時保存領域を利用できません。");
  }

  storage.setItem(PRODUCT_UPDATE_STORAGE_KEY, JSON.stringify(product));
};

/**
 * URLの商品UUIDと一致する引き継ぎ済み商品を取得する
 */
export const loadProductForUpdate = (productUuid: string): Product | null => {
  const storage = getSessionStorage();
  ...
  try {
    const storedValue = storage.getItem(PRODUCT_UPDATE_STORAGE_KEY);
    ...
    return product.productUuid === productUuid ? product : null;
  } catch {
    return null;
  }
};
