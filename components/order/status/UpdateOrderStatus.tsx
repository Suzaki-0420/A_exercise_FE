...
export const UpdateOrderStatus = ({
  ...
  const {
    ...
    closeToast,
  } = useUpdateOrderStatus(orderUuid, currentStatusId, currentStatusName);
   ...
   * 表示用に変換する
   */
  const formattedConfirmOrderDate = confirmedOrder?.orderDate
    ? new Date(confirmedOrder.orderDate).toLocaleString("ja-JP")
    : formattedOrderDate;
  ...
  if (isLoading && !order) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-white">
        <p className="text-gray-600">注文情報を読み込んでいます...</p>
      </main>
    );
  }
  ...
  return (
    <>
          ...
          {!order ? (
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <div className="grid grid-cols-[180px_1fr] items-start gap-4">
                <p className="text-right font-semibold">購入日時</p>
              ...
              {/* 顧客アカウント名 */}
              <div className="grid grid-cols-[180px_1fr] items-start gap-4">
                <p className="text-right font-semibold">顧客アカウント名</p>

                <p>{order.customerAccountName || "-"}</p>
              </div>

              {/* 注文ID */}
              <div className="grid grid-cols-[180px_1fr] items-start gap-4">
                <p className="text-right font-semibold">注文ID</p>
              ...
              {/* 注文内容 */}
              <div className="grid grid-cols-[180px_1fr] items-start gap-4">
                <p className="text-right font-semibold">注文内容</p>

                <p className="whitespace-pre-line break-words">
                  {order.orderContent || "注文内容がありません"}
                </p>
              </div>

              {/* 現在のステータス */}
              <div className="grid grid-cols-[180px_1fr] items-start gap-4">
                <p className="text-right font-semibold">現在のステータス</p>

                <p>{order.currentStatusName || "-"}</p>
              </div>
              ...
              <div className="flex justify-center gap-6 pt-8">
                ...
                <button
                  ...
                  disabled={
                    ...
                    hasValidationErrors ||
                    selectedStatusId === 0 ||
                    selectedStatusId === order.currentStatusId ||
                    orderStatusOptions.length === 0
                  }
      ...
      {/* 確認モーダル */}
      {isConfirmOpen && order && confirmedOrder && (
        <div
          ...
          >
            ...
            <dl className="space-y-4">
              <div className="grid grid-cols-[150px_1fr] gap-4 border-b border-gray-200 pb-3">
                <dt className="font-semibold">購入日時</dt>

                <dd>{formattedConfirmOrderDate}</dd>
              </div>

              <div className="grid grid-cols-[150px_1fr] gap-4 border-b border-gray-200 pb-3">
                <dt className="font-semibold">顧客アカウント名</dt>

                <dd>
                  {confirmedOrder.customerAccountName ||
              ...
              <div className="grid grid-cols-[150px_1fr] gap-4 border-b border-gray-200 pb-3">
                <dt className="font-semibold">注文ID</dt>

                <dd className="break-all">
                  {confirmedOrder.orderId || order.orderId || "-"}
                </dd>
              </div>

              <div className="grid grid-cols-[150px_1fr] gap-4 border-b border-gray-200 pb-3">
                <dt className="font-semibold">現在のステータス</dt>

                <dd>
                  {confirmedOrder.currentStatusName ||
              ...
              <div className="grid grid-cols-[150px_1fr] gap-4 border-b border-gray-200 pb-3">
                <dt className="font-semibold">新しいステータス</dt>

                <dd className="font-bold text-green-700">
                  {confirmedOrder.newStatusName || "-"}
                </dd>
              </div>
            </dl>
      ...
      {isToastVisible && (
        ...
        >
          ...
          <p className="font-semibold">注文ステータスを更新しました。</p>
